...
connected(cast, {done, ChallengeePid, WonOrLost}, Data = #state{challengee_pid = ChallengeePid, challengee_won=undefined}) ->
    ...
    check_winner(NewData);
connected({call, From}, {subscribe, Channel, Pid, ProtocolVersion, SuccessFun}, Data = #state{subscribers=Subs, events=Events}) ->
    ...
    case Data#state.challenger_pilot /= undefined andalso Data#state.challengee_pilot /= undefined andalso Data#state.confirmed_rand /= undefined of
        true ->
            ...
            true;
        false ->
            lager:info("match not started yet (challenger ~p, challenge ~p, seed ~p), having subscriber wait", [Data#state.challenger_pilot /= undefined, Data#state.challengee_pilot /= undefined, Data#state.confirmed_rand /= undefined]),
            false
    end,
